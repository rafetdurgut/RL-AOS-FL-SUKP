...
chi_scores = chi2(X,y)
sns.barplot(feature_names, chi_scores[0],ax=axes[0]).set_title(f"Chi-square scores", fontdict={'fontsize':18}, pad=12);
...
clf2.fit(X, y)
chi_scores = chi2(X,y)
sns.barplot(feature_names, chi_scores[0],ax=axes[1])
...
chi_scores = chi2(X,y)
sns.barplot(feature_names, chi_scores[0],ax=axes[2])
plt.xlabel('Features')
plt.ylabel('Values')
plt.savefig(f"figs/CH-FI.pdf")
plt.show()
plt.close()
