...
feature_importance=[abs(i) for i in clf.coef_[0]]

sns.barplot(feature_names, feature_importance,ax=axes[0]).set_title(f"SVM - Feature Coefficients")
...
clf2.fit(X, y)
feature_importance=[abs(i) for i in clf2.coef_[0]]

sns.barplot(feature_names, feature_importance,ax=axes[1])
...
clf3.fit(X, y)
feature_importance=[abs(i) for i in clf3.coef_[0]]

sns.barplot(feature_names, feature_importance,ax=axes[2])

plt.ylabel("Coefficients")
plt.xlabel("Features")
plt.savefig(f"figs/SVC-FI-SUKP.pdf")
plt.show()
plt.close()
